...
int main(int argc, char** argv) {
	...
	redis_client.addEigenToReadCallback(0, DESIRED_DEVICE_POSITION_KEY, device_desired_position);
	redis_client.addEigenToReadCallback(0, DESIRED_DEVICE_LINEAR_VELOCITY_KEY, device_desired_linear_velocity);
	...
	redis_client.addEigenToWriteCallback(0, DEVICE_COMMANDED_FORCE_KEY, device_command_force);
	...
	while (runloop) 
	{
		...
		// compute control
		if(device_desired_position.norm() > workspace_radius)
		{
			device_desired_position *= workspace_radius/device_desired_position.norm();
		}

		device_command_force = - kp * (device_position - device_desired_position) - kv * (device_velocity - device_desired_linear_velocity);
