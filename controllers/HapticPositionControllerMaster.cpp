...
int main(int argc, char** argv) {
	...
	if(argc < 2)
	{
		cout << "Please enter the device name as an argument ('device0' or 'device1' or 'device0-remote')" << endl;
		return -1;
	}
	...
	{
		cout << "argument must be 'device0' or 'device1' or 'device0-remote'" << endl;
		return -1;       
	}
	...
	auto redis_client_remote = RedisClient();
	redis_client_remote.connect(remote_ip);
	...
	// redis_client_local.setEigenMatrixJSON(DESIRED_DEVICE_POSITION_KEY, device_desired_position);
	// redis_client_local.setEigenMatrixJSON(DESIRED_DEVICE_LINEAR_VELOCITY_KEY, device_desired_linear_velocity);

	Vector3d device_position = redis_client_local.getEigenMatrixJSON(DEVICE_POSITION_KEY);
	Vector3d device_velocity = redis_client_local.getEigenMatrixJSON(DEVICE_TRANS_VELOCITY_KEY);
	...
	redis_client_remote.addEigenToWriteCallback(0, DESIRED_DEVICE_POSITION_KEY, device_desired_position);
	redis_client_remote.addEigenToWriteCallback(0, DESIRED_DEVICE_LINEAR_VELOCITY_KEY, device_desired_linear_velocity);
